...
def hmm_fit_em_setup(num_trials=5, num_timesteps=100, latent_dim=3, emissions_dim=10, num_iters=100):
    true_hmm = HMM(emissions_dim, latent_dim, dynamics="gaussian", emissions="gaussian")
    states, data = sample_hmm(true_hmm, num_trials=num_trials, time_bins=num_timesteps)
    test_hmm = HMM(emissions_dim, latent_dim, dynamics="gaussian", emissions="gaussian")
