...
def do_fivo_sweep(_param_vals,
    ...
    # Compute the mean of the log marginal - THIS IS THE FIVO BOUND.
    _fivo_bound = np.mean(_smc_posteriors.log_normalizer)

    return - _fivo_bound, _smc_posteriors


def _define_initial_distribution(_model, _proposal):
    """
    ...
    if _proposal is None:
        ...
        # If there is no proposal, just use the BPF.
        def _initialization_distribution(_subkey, _num_particles, *_args):
            """
            ...
            Returns: (Distribution over stochastic part of state,
                      Samples from initial distribution of p state,
                      Deterministic part of p state,
                      Initial state of q (= None in BPF case)).

            """
            _dist = _model.initial_distribution()

            # If there is no deterministic return type, then inscribe none...
            if type(_dist) is not tuple:
                _det = None
            else:
                _det = _dist[1]
                _dist = _dist[0]

            _particles = _dist.sample(seed=_subkey, sample_shape=(_num_particles, ), )
            return (_dist, _particles, _det, None)

    else:

        # If there is a proposal, hijack the zeroth proposal as the initialization distribution.
        def _initialization_distribution(_subkey, _num_particles, *_args):
            """
            ...
            """
            _dist, _det, _q_state = _proposal(np.zeros(_model.latent_dim, ), 0, _model.initial_distribution(), None)
            _particles = _dist.sample(seed=_subkey, sample_shape=(_num_particles, ), )
            return (_dist, _particles, _det, _q_state)

    return _initialization_distribution
...
def _do_single_fivo_sweep(_param_vals,
    ...
    initialization_distribution = _define_initial_distribution(_model, _proposal)
