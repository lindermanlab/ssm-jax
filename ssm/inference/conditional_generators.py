...
class IndependentBernoulliGenerator(nn.Module):
    ...
    @classmethod
    def from_params(cls, dummy_input, dummy_output, trunk_fn=None):
        """
        Build a nn.Module that implements a conditional Gaussian density.  This function basically wraps a generator and
        allows for more flexible definition of link functions.
        ...
        # Type check to make sure that the mean function produces the right size.
        _dim = trunk_fn.features[-1] if hasattr(trunk_fn.features, '__iter__') else trunk_fn.features
        assert output_flat_dim == _dim, \
            'Error: trunk function dimensions not equal to the target output dimensions.'

        return cls(trunk_fn, unravel_output, input_flat_dim)

    def tree_flatten(self):
        children = (self.trunk_fn, self.unravel_output)
        aux_data = None
        return children, aux_data
    ...
    def _call_single(self, inputs):
        ...
        # Unravel the output to the original shape.
        log_odds_output_shaped = self.unravel_output(trunk_output)

        return log_odds_output_shaped
