...
def forward_pass(initial_potentials, unary_potentials, pairwise_potentials):
    """
    ...
    Returns:
        ...
        discrete states.
    """
    assert initial_potentials.ndim == 1 and unary_potentials.ndim == 2
    num_states = len(initial_potentials)
    num_timesteps = len(unary_potentials)
    assert unary_potentials.shape[1] == num_states

    if pairwise_potentials.ndim == 2:
        # Stationary (fixed) transition probabilities
        assert pairwise_potentials.shape == (num_states, num_states)
        return _forward_pass_stationary(initial_potentials,
                                        unary_potentials,
                                        pairwise_potentials)

    elif pairwise_potentials.ndim == 3:
        # Time-varying transition probabilities
        assert pairwise_potentials.shape == \
            (num_timesteps - 1, num_states, num_states)
        return _forward_pass_nonstationary(initial_potentials,
                                           unary_potentials,
                                           pairwise_potentials)
...
def _forward_pass_stationary(initial_potentials,
                             unary_potentials,
                             pairwise_potentials):
    ...
    return spsp.logsumexp(alpha_T + unary_potentials[-1])


def _forward_pass_nonstationary(initial_potentials,
                                unary_potentials,
                                pairwise_potentials):
    ...
    return spsp.logsumexp(alpha_T + unary_potentials[-1]), alphas
...
class DiscreteChain:
    ...
    def __init__(self,
        ...
        self._num_timesteps, self._num_states = unary_potentials.shape[-2:]
        self._unary_potentials = unary_potentials

        assert initial_potentials is not None and initial_potentials.ndim >= 1
        assert initial_potentials.shape[-1] == self._num_states
        self._initial_potentials = initial_potentials

        assert pairwise_potentials is not None
        assert pairwise_potentials.shape[:-3] == unary_potentials.shape[:-2]
        assert pairwise_potentials.shape[-3:] == \
            (self._num_timesteps - 1, self._num_states, self._num_states)
        ...
        super().__init__(
            ...
            parameters=dict(unary_potentials=self._unary_potentials,
    ...
    def message_passing(self):
        ...
        def _message_passing_single(initial_potentials,
                                    unary_potentials,
                                    pairwise_potentials):
            f = value_and_grad(forward_pass, argnums=(1, 2), has_aux=True)
    ...
    def _log_prob(self, data, **kwargs):
        T, K = self._num_timesteps, self._num_states
        batch_shape = self.batch_shape
        flat_data = data.reshape(-1, T, K)
